...
namespace Funcular.Data.Orm.SqlServer.Tests
{
    ...
    public class SqlDataProviderPerformanceTests
    {
        ...
        private void CleanupTestRecords(string firstName, string lastName)
        {
            ...
            sqlConnection.Open();
            using var command = new SqlCommand("DELETE FROM person WHERE first_name = @firstName AND last_name = @lastName", sqlConnection);
        ...
        private void TestConnection()
        {
            ...
            {
                throw new InvalidOperationException("Connection failed. Ensure funky_db exists and configure FUNKY_CONNECTION environment variable if not using localhost.\r\n\r\n" + ex);
            }
        }
