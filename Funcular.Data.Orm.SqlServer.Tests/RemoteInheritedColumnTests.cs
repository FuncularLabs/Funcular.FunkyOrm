    ...
    public class RemoteInheritedColumnTests
    {
        ...
        private void EnsureSchema()
        {
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                drop.CommandText = @"
                    IF OBJECT_ID('funky_inh_child_b','U') IS NOT NULL DROP TABLE funky_inh_child_b;
                    IF OBJECT_ID('funky_inh_child_d','U') IS NOT NULL DROP TABLE funky_inh_child_d;
                    IF OBJECT_ID('funky_inh_parent','U') IS NOT NULL DROP TABLE funky_inh_parent;";
                drop.ExecuteNonQuery();

                var create = conn.CreateCommand();
                create.CommandText = @"
                    CREATE TABLE funky_inh_parent (id INT IDENTITY(1,1) PRIMARY KEY, uid NVARCHAR(40) NULL, display_name NVARCHAR(100) NULL);
                    CREATE TABLE funky_inh_child_b (id INT IDENTITY(1,1) PRIMARY KEY, inh_parent_id INT NULL);
                    CREATE TABLE funky_inh_child_d (id INT IDENTITY(1,1) PRIMARY KEY, inh_parent_id INT NULL);
        ...
        {
            // Pre-fix: threw InvalidOperationException / SqlException "Invalid object name 'inhparentbase'"
