...
All four work in `Get<T>`, `Query<T>`, and `GetList<T>`; the scalar three (`[JsonPath]`, `[SqlExpression]`, `[SubqueryAggregate]`) — and `[RemoteProperty]`/`[RemoteKey]` — also resolve in **WHERE predicates**, **ORDER BY**, and **aggregate filters** (`Count`/`Any`/`Sum`/…). (One exception: filtering `Count`/`Sum`/`Average` by a *reverse* one-to-many `[RemoteKey]`/`[RemoteProperty]` throws — the join would fan out; aggregate in memory instead.) See the [Usage Guide](Usage.md) for detailed documentation, generated SQL, and parameter tables.
