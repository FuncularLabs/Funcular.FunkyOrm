...
### 6.1 Implementation Details & Bug Fix (v3.5.1)

**Two integration points require the `remotePropertyMap`:**

1. **`VisitMember` (binary expressions)** — The `_remotePropertyMap` is checked first when a member 
   expression references a parameter property (e.g., `p.Priority == "high"`). This was implemented 
   correctly in the initial implementation.

2. **`VisitMethodCall` (method call expressions)** — The `getColumnName` delegate passed to 
   `TranslateMethodCall` must ALSO check `_remotePropertyMap` before falling back to `GetColumnName`. 
   This was **missed** in the initial implementation, causing method-call predicates like 
   `p.ClientName.Contains("Acme")` or `list.Contains(p.Priority)` to emit plain column names 
   instead of JSON accessors.

3. **`SqlLinqQueryProvider` aggregate paths (Any/All/Count)** — These create `WhereClauseVisitor` 
   directly without calling `GenerateWhereClause`. They must also resolve remote joins and pass 
   `PropertyToColumnMap` to the visitor. This was **missed** in the initial implementation.

**Key lesson:** Any code path that resolves a property name to a SQL column expression must consult 
the `PropertyToColumnMap` (passed as `remotePropertyMap`). This includes both the `VisitMember` path 
for simple binary comparisons AND the `getColumnName` delegate used by expression translators for 
method calls (Contains, StartsWith, EndsWith, IN clauses).
