...
namespace Funcular.Data.Orm.PostgreSql.Tests
{
    ...
    /// </summary>
    [TestClass]
    public class PostgreSqlRemoteExplicitTargetAuthorityTests
    {
        ...
        private void EnsureSchema()
        {
            using (var conn = new NpgsqlConnection(_connectionString))
            {
                ...
                drop.CommandText = @"
                    DROP TABLE IF EXISTS funky_b2_child;
                    DROP TABLE IF EXISTS funky_b2_target;";
                drop.ExecuteNonQuery();

                var create = conn.CreateCommand();
                create.CommandText = @"
                    CREATE TABLE funky_b2_target (id SERIAL PRIMARY KEY, target_name VARCHAR(50) NULL);
                    CREATE TABLE funky_b2_child (id SERIAL PRIMARY KEY, lookup_id INT NULL);
