...
{
    /// <summary>
    /// Regression test for the cold-cache remote-column bug: a [RemoteProperty]'s target column was
    ...
    /// </summary>
    [TestClass]
    public class PostgreSqlRemoteColdCacheTests
    {
        ...
        private void EnsureSchema()
        {
            using (var conn = new NpgsqlConnection(_connectionString))
            {
                ...
                drop.CommandText = @"
                    DROP TABLE IF EXISTS funky_cold_source;
                    DROP TABLE IF EXISTS funky_cold_target;";
                drop.ExecuteNonQuery();

                var create = conn.CreateCommand();
                create.CommandText = @"
                    CREATE TABLE funky_cold_target (id SERIAL PRIMARY KEY, cold_value VARCHAR(100) NULL);
                    CREATE TABLE funky_cold_source (id SERIAL PRIMARY KEY, cold_target_id INT NULL);
        ...
        {
            // Pre-fix (cold): the remote column resolved to naive "coldvalue" → PostgresException "column ... does not exist".
