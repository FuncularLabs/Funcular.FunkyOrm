...
> **Target Audience**: AI Agents (GitHub Copilot, Cursor, Gemini, GPT, Claude, etc.) assisting developers who use the `Funcular.Data.Orm` NuGet package.
>
> **Package**: `Funcular.Data.Orm` | **Namespace**: `Funcular.Data.Orm.SqlServer`
...
2.  **NO `DbContext`**: There is no Unit of Work or Context pattern.
    *   *Correct*: Use `SqlServerOrmDataProvider` as a singleton or transient service.
...
*   **Anti-Pattern**: Do NOT use `SqlConnection`/`SqlCommand` to bypass these checks.
...
**Recommended Suffixes**: `Dto`, `View`, `Vm`, `Model`, `Item`, `Summary`, `Detail`

---

## ?? Reserved Words & Schema

*   **Reserved Words**: Do NOT manually bracket reserved words (e.g., `User`, `Order`). The framework handles this automatically.

*   **Schema Changes**: If the DB schema changes, the application **MUST** be restarted to clear the reflection cache.
...
```csharp
// Provider Setup (singleton recommended)
var provider = new SqlServerOrmDataProvider(connectionString);

// CRUD Operations
var id = provider.Insert<Person, int>(person);      // Insert, returns typed ID
var entity = provider.Get<Person>(id);              // Get by ID
provider.Update(entity);                            // Update
provider.BeginTransaction();                        // Required for Delete
provider.Delete<Person>(id);                        // Delete by ID
provider.Delete<Person>(p => p.IsInactive);         // Delete by predicate
provider.CommitTransaction();

// Querying
var list = provider.Query<Person>()
    .Where(p => p.IsActive)
    .OrderBy(p => p.LastName)
    .Take(10)
    .ToList();

// Async versions: GetAsync, InsertAsync, UpdateAsync, DeleteAsync, QueryAsync
```
