...
namespace Funcular.Data.Orm.SqlServer
{
    ...
    public partial class SqlServerOrmDataProvider : ISqlDataProvider, IDisposable
    {
        ...
        protected internal SqlQueryComponents<T> CreateSelectQuery<T>(Expression<Func<T, bool>> whereExpression) where T : class, new()
        {
            ...
            elements.OrderByClause = GenerateOrderByClause(whereExpression).OrderByClause;
            return elements;
            // return new SqlQueryComponents<T>(whereExpression, selectClause, elements.WhereClause, string.Empty, elements.SqlParameters);
        }

        protected internal SqlQueryComponents<T> GenerateWhereClause<T>(Expression<Func<T, bool>> expression, SqlQueryComponents<T>? commandElements = null) where T : class, new()
        {
        ...
        protected internal SqlQueryComponents<T> GenerateOrderByClause<T>(Expression<Func<T, bool>> expression, SqlQueryComponents<T>? commandElements = null) where T : class, new()
        {
            ...
            if (commandElements == null)
            {
                commandElements = new SqlQueryComponents<T>(expression, string.Empty, string.Empty, visitor.OrderByClause, visitor.Parameters);
            }
            else
            {
            ...
            return
                commandElements; //new SqlQueryComponents<T>(expression, string.Empty, visitor.OrderByClause, string.Empty, visitor.Parameters);
        }
