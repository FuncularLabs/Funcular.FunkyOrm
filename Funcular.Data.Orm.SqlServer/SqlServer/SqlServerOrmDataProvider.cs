    ...
    public partial class SqlServerOrmDataProvider : OrmDataProvider, ISqlOrmProvider
    {
        ...
        private string LookupProcedureName(string normalized)
        {
            ...
            using (var command = BuildSqlCommandObject(
                       "SELECT name FROM sys.procedures WHERE REPLACE(LOWER(name), '_', '') = @normalized",
                       scope.Connection,
                       new[] { new SqlParameter("@normalized", normalized) }))
            {
                InvokeLogAction(command);
        ...
        /// <summary>Converts normalized parameters to native <see cref="SqlParameter"/>s (re-adding the <c>@</c> prefix).</summary>
        private static ICollection<SqlParameter> BuildProcedureParameters(IReadOnlyList<NormalizedParameter> normalized)
        {
            var list = new List<SqlParameter>(normalized.Count);
            foreach (var np in normalized)
            {
                var parameter = new SqlParameter("@" + np.Name, np.Value ?? DBNull.Value)
                {
        ...
        private static void BackPopulateOutputParameters(SqlCommand command, IReadOnlyList<NormalizedParameter> normalized)
        {
            foreach (var np in normalized)
            {
        ...
        internal ResolvedRemoteJoinInfo ResolveRemoteJoins<T>(string tableName)
        {
            ...
            foreach (var prop in jsonCollectionProperties)
            {
                ...
                string childTable = GetTableNameForType(attr.SourceType);
                string childFkPropInfo = attr.SourceType.GetProperty(attr.ForeignKey)?.Name;
                string childFkCol = childFkPropInfo != null
                    ? GetCachedColumnName(attr.SourceType.GetProperty(attr.ForeignKey))
                    : Dialect.EncloseIdentifier(attr.ForeignKey.ToLower());
                var pk = GetCachedPrimaryKey<T>();
                string parentPk = $"{tableName}.{GetCachedColumnName(pk)}";
        ...
        /// If a property name matches an already-resolved expression in <paramref name="existingMap"/>,
        /// that expression is substituted inline.
        /// </summary>
        private string ResolveExpressionTokens(string expression, Type entityType, string tableName, Dictionary<string, string> existingMap)
        {
            return System.Text.RegularExpressions.Regex.Replace(expression, @"\{(\w+)\}", match =>
            {
                ...
                // If the property was already resolved (e.g., another computed column), substitute inline
                if (existingMap.TryGetValue(propName, out string existing))
                    return existing;

                // Otherwise resolve to a qualified column name
                ...
                // Fallback: emit as-is (will likely cause a SQL error, which is appropriate)
                return match.Value;
            });
        }

        /// <summary>
        /// Gets the resolved table name for a type, using the same caching as GetTableName.
        /// </summary>
        ...
        protected internal string CreateGetOneOrSelectCommandText<T>(dynamic key = null) where T : class, new()
        {
            ...
            {
                // Prefix main table columns to avoid ambiguity when JOINs are present,
                // or when extra columns (e.g., JSON extractions) reference the same table.
